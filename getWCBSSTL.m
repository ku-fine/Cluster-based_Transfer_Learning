function [W, K, n1, n2]=getWCBSSTL(Xs, Xt, mu, lambda, dim, kind, p1, p2, p3, labels1, labelyt)
%Xs：源域数据
%Xt：目标域数据，与Xs行数相同
...
%%%%%%%%%%% 计算包含目标域伪标签类的类内距离 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    ind=1:n1+n2;
    count=0;
    labelyt=labelyt+max(labelyt);
    alllabels=[labels1,labelyt];
    for i=min(alllabels):max(alllabels)
        a=(alllabels==i);
        s=sum(a);
        ind1=ind(a);
        t1=zeros(n1+n2);
